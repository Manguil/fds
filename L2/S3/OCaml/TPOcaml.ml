...
(*** Eval ***)
let f = Or ( And ( And ( Equ ( Imp ( Symb "m1" ,
                                     And ( Top , Symb "m2") ) ,
                               And ( Bot , Not ( Symb "m2" ) ) ) ,
                         Symb "m3" ),
                   ( Not ( Symb "m4" ) ) ) ,
             Imp ( Symb "m2" , Imp ( Symb "m2" , Symb "m1" ) ) ) ;;
let j = [("m2",Zero);("m1",Zero);("m3",Zero);("m4",Zero)];;
(*** test Q3 ***)
prof f;;
(*** test Q4 ***)
sp f;;
...
valide f ;;
(*** test Q17 ***)
equivalent1 (Symb "l") (And (Top, Symb "n"));;
equivalent2 (And (Symb "l",Not (Symb "l"))) (Not (Imp (Symb "n", Symb "n")));;
(*** test Q18 ***)
consequence2 (Not f) (And (Symb "m2", Not (Symb "m1")));;
(*** test Q22 ***)
consequence [Top ; Bot] Top ;;
(*** test Q23 ***)
consequenceV [ f ; (Not (Or (Symb "m2",Symb "m1")))] (Imp (Symb "m3", Symb "m4"));;
(*** test Q24 ***)
consequenceI [ f ; (Not (Or (Symb "m2",Symb "m1"))) ; (And (Symb "m3", Not (Symb "m4"))) ]
  (Equ (Symb "m2", Symb "m4"));;

(** nbModele **)
let liM fbf = listeModele fbf (ensInt (sp fbf))
let rec nbModeleInt fbf = function
  |[] -> 0
  |t::q -> if (t) then 1 + nbModeleInt fbf q else nbModeleInt fbf q
;;
let nbModele fbf = nbModeleInt fbf (liM fbf)
;;

nbModele (And(Symb "a", Symb "b")) ;;

nbModele (And(Symb "a", Not(Symb "a"))) ;;

nbModele (Or(Symb "a", Not(Symb "a"))) ;;
...
d : je suis dehors
a : chien aboie
q : chien voit quelqu'un
*)
let ha = Imp(Symb "p", Not(Symb "j"));; 
let hb = Imp(Symb "j", Symb "d");;
let hc = Imp(And(Symb "q",Not(Symb "d")), Symb "a");;
let hd = Symb "a";;
(*
  let evalProbleme = consequence 
*)