...
public class SyntheseMeteo {
    ...
    public int moisLePlusChaud() {
        int nbTemperatureMax = 0;
        float[][] tabTempMax = new float[12][2];
        for (int i = 0; i < 12; i++){
            if (this.temperatureMensuelleMax() == tmax[i]){
                tabTempMax[nbTemperatureMax]= {tmax[i],i};
                nbTemperatureMax++;
            }
        }
        return 2;
     ...
     * //return Mois.values()[numMois-1]; // Cette instruction aurait suffit mais la
     * comprenez-vous ?
     * switch (numMois){
     ...
     * //return Mois.JANVIER.ordinal()+1; // Cette instruction aurait suffit mais la
     * comprenez-vous ?
     * switch (mois){
